...
export default function ProfilePage() {
  ...
  const mobile = userData?.mobile ?? sessionUser?.mobile ?? 'N/A';
  ...
  const dailyGoals = [
    { label: "Coins Collected Today:", progress: Math.min((coins / 1000) * 100, 100), current: coins, total: 1000 },
    { label: "XP Status:", progress: leveling.isMaster ? 100 : 0, current: leveling.isMaster ? 1 : 0, total: 1 },
  ];

  const stats = [
    { label: "Total XP Collected:", value: isAdmin ? "UNLIMITED" : Math.floor(xp).toLocaleString() },
    { label: "Phone Number:", value: mobile },
    { label: "Current Rank:", value: leveling.isMaster ? (isAdmin ? "System Master" : "Glow Master") : "Novice Earner" },
  ];
        ...
        <header className="flex flex-col items-center">
          <div className="relative">
            ...
            <Avatar className="h-24 w-24 border-4 border-glowearn-navy">
              <AvatarImage src={userData?.avatarUrl || "https://picsum.photos/seed/gold-avatar/200/200"} />
            ...
            {leveling.isMaster && (
              <div className="absolute -right-12 top-4 flex items-center gap-1 bg-glowearn-gold px-2 py-1 rounded-full border border-glowearn-gold/20 shadow-lg">
                ...
                <span className="text-glowearn-navy font-black text-[9px] uppercase">{isAdmin ? 'Admin' : 'Master'}</span>
              </div>
            )}
          </div>
          ...
          <div className="text-center mt-6">
            <h1 className="text-white font-headline text-2xl font-black uppercase tracking-tight">
              {name} <span className="text-glowearn-gold ml-1">Lvl {leveling.level}</span>
            </h1>
            <p className="text-white/70 font-bold text-[11px] uppercase tracking-[0.2em] mt-1">
              Member ID: {sessionUser?.id?.slice(-6) || '---'}
            </p>
          </div>
        </header>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-[10px] text-white/40 font-bold uppercase">
                    {isAdmin ? 'MAX' : `${Math.floor(leveling.currentXp)} / ${Math.floor(leveling.xpForNext)} XP`}
                  </span>
                </div>
                <Progress 
                  ...
                  {dailyGoals.map((goal, i) => (
                    <div key={i} className="space-y-2">
                      <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                        ...
                        <span className="text-white">{Math.floor(goal.current)}/{goal.total}</span>
                      </div>
